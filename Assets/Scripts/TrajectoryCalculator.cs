...
{
    public RootMotionCurves GetTrajectoryCurves(RootMotionCurves original, float takeOffTime, float landTime, float gravity = -9.81f)
    {
        List<float> keyTimes = new List<float>();

        for (int i = 0; i < original.rootTXCurve.length; i++)
        {
            float curveTime = original.rootTXCurve[i].time;
            
            if(curveTime >= takeOffTime && curveTime <= landTime)
                keyTimes.Add(original.rootTXCurve[i].time);
        }

        for (int i = 0; i < original.rootTYCurve.length; i++)
        {
            float curveTime = original.rootTYCurve[i].time;
        ...
        for (int i = 0; i < original.rootTZCurve.length; i++)
        {
            float curveTime = original.rootTZCurve[i].time;
        ...
        Vector3 takeOffPosition = new Vector3(original.rootTXCurve.Evaluate(takeOffTime), original.rootTYCurve.Evaluate(takeOffTime), original.rootTZCurve.Evaluate(takeOffTime));
        Vector3 landPosition = new Vector3(original.rootTXCurve.Evaluate(landTime), original.rootTYCurve.Evaluate(landTime), original.rootTZCurve.Evaluate(landTime));
    ...
    float GetLateralTrajectory(float time, float duration, float distance)
    {
        return distance * time / duration;
    }

    float GetVerticalTrajectory(float time, float duration, float gravity)
    {
